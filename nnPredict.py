...
#for predictions
predictOp = tf.arg_max(tf.nn.softmax(ffOp), 1, name="Predictions")
...
def train(numberOfIterations):
    ...
    for i in range(numberOfIterations):
        #run training
        #need to use feed_dict to put data items into placeholder spots
        sess.run(gradientOp, feed_dict={input: trainX, label: trainY})          #label is trainX b/c autoencoder
        #compute average cost for printing
        avgCost = sess.run(lossOp, feed_dict={input: trainX, label: trainY})    #label is trainX b/c autoencoder
        #print
        print "iteration %s with average cost of %s" %(str(i),str(avgCost))
...
def test():
    #predicting on first 500 items
    # for i in range(testX.shape[0]):
        prediction = sess.run(predictOp, feed_dict={input: testX, label: testY})
        print "correct: %s; predicted: %s" %(testY, prediction)
